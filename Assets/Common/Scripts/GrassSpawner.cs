...
public class GrassSpawner : MonoBehaviour
{
	...
	{
		// There will only be intersections if the distance between circles is less than the sum of their radii and more than the difference between their radii
	...
	private Vector2 CalcIntersectionPoint(Vector2 circleACenter, Vector2 centerDelta, float centerDistance, float radiusA, float radiusB)
	{
		...
		Vector2 pointA = new Vector2(lDivD * centerDelta.x + hDivD * centerDelta.y + circleACenter.x, lDivD * centerDelta.y - hDivD * centerDelta.x + circleACenter.y);
	...
	void Start()
    {
		...
		{
			GameObject.Instantiate(prefab, new Vector3(point.x, 0, point.y) + centerPos, Quaternion.Euler(0, Random.value * 360, 0), transform);
		}
	}
